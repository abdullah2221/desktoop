...
export interface BankAccountPayload {
  ...
  code: string;
  name: string;
  account_type: 'Cash' | 'Bank' | 'EasyPaisa' | 'JazzCash';
...
export class BankAccountRepository {
  ...
  static update(payload: BankAccountPayload & { id: string }) {
    const db = getDatabase();
    const info = db.prepare(`
    ...
    if (info.changes > 0) {
      AuditLogRepository.write({ action: 'BANK_ACCOUNT_UPDATE', details: `Bank/Cash account ${payload.id} updated` });
    }
  ...
  static deactivate(id: string) {
    ...
    if (info.changes > 0) {
      AuditLogRepository.write({ action: 'BANK_ACCOUNT_DEACTIVATE', details: `Bank/Cash account ${id} deactivated` });
    }

    return info.changes > 0;
  }

  static mapPaymentMethod(paymentMethod: 'Cash' | 'Bank' | 'EasyPaisa' | 'JazzCash' | 'Card' | 'Cheque', accountId: string | null) {
    const db = getDatabase();
    const info = db.prepare(`
    ...
    if (info.changes > 0) {
      AuditLogRepository.write({ action: 'PAYMENT_METHOD_MAP_UPDATE', details: `${paymentMethod} mapped to ${accountId || 'NULL'}` });
    }
