    ...
    static create(payload) {
      ...
      `);
            insertPurchase.run({
                ...
                tax_code: purchaseData.tax_code || TaxRepository_1.TaxRepository.getDefaultTaxCode('purchase'),
      ...
      `);
            for (const item of purchaseData.items) {
                ...
                const product = ProductRepository_1.ProductRepository.getById(item.product_id);
                if (product) {
          ...
          `).run(newStock, item.unit_cost, item.product_id);
            ...
            const supplier = SupplierRepository_1.SupplierRepository.getById(purchaseData.supplier_id);
            if (supplier) {
        ...
        `).run({
          ...
          `).run({
            ...
            try {
                AccountingPostingService_1.AccountingPostingService.postPurchase({
                    purchaseId,
                    date: purchaseData.date || new Date().toISOString(),
