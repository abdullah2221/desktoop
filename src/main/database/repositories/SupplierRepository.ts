...
export class SupplierRepository {
  ...
  static create(supplier: any) {
    ...
    if (info.changes > 0) {
      AuditLogRepository.write({ action: 'SUPPLIER_CREATE', details: `Supplier ${id} created` });
    }

    return info.changes > 0 ? { success: true, id } : { success: false };
  }

  static update(supplier: any) {
    const db = getDatabase();
    const stmt = db.prepare(`
      UPDATE suppliers 
      SET 
    ...
    if (info.changes > 0) {
      AuditLogRepository.write({ action: 'SUPPLIER_UPDATE', details: `Supplier ${supplier.id} updated` });
    }
  ...
  static deactivate(id: string) {
    ...
    if (info.changes > 0) {
      AuditLogRepository.write({ action: 'SUPPLIER_DEACTIVATE', details: `Supplier ${id} deactivated` });
    }
    return info.changes > 0;
  }

  static getLedger(supplierId: string) {
    const db = getDatabase();
