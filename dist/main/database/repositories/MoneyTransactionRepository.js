...
exports.MoneyTransactionRepository = void 0;
...
class MoneyTransactionRepository {
    ...
    static createDeposit(payload) {
        return this.createSingle(payload, 'DEPOSIT', 1);
    }
    static createWithdrawal(payload) {
        return this.createSingle(payload, 'WITHDRAWAL', -1);
    }
    static createBankCharge(payload) {
    ...
    static createTransfer(payload) {
        ...
        const tx = db.transaction((data) => {
            const source = db.prepare('SELECT * FROM cash_bank_accounts WHERE id = ?').get(data.from_account_id);
            const destination = db.prepare('SELECT * FROM cash_bank_accounts WHERE id = ?').get(data.to_account_id);
            if (!source || !destination)
                throw new Error('Source or destination account not found.');
            if (data.amount <= 0)
                throw new Error('Transfer amount must be greater than zero.');
            if (source.current_balance < data.amount)
                throw new Error('Insufficient source account balance for transfer.');
      ...
      `).run({
            ...
            AuditLogRepository_1.AuditLogRepository.write({ action: 'MONEY_TRANSFER', details: `Transfer ${data.amount} from ${data.from_account_id} to ${data.to_account_id}` });
            return { success: true, id: outId };
        });
        return tx(payload);
    }
    static markCleared(transactionId, cleared = true) {
        const db = (0, connection_1.getDatabase)();
    ...
    static createSingle(payload, type, direction) {
        const db = (0, connection_1.getDatabase)();
        const tx = db.transaction((data) => {
            const account = db.prepare('SELECT * FROM cash_bank_accounts WHERE id = ?').get(data.account_id);
            if (!account)
      ...
      `).run({
            ...
            AuditLogRepository_1.AuditLogRepository.write({ action: `MONEY_${type}`, details: `${type} ${data.amount} in ${data.account_id}` });
            return { success: true, id };
        });
        return tx(payload);
    }
}
exports.MoneyTransactionRepository = MoneyTransactionRepository;
