...
import { MoneyTransactionRepository } from './MoneyTransactionRepository';
...
export class BankReconciliationRepository {
  ...
  static createWorksheet(payload: {
    ...
    const account = db.prepare('SELECT opening_balance FROM cash_bank_accounts WHERE id = ?').get(payload.account_id) as { opening_balance: number } | undefined;
    ...
    `).all(payload.account_id, payload.end_date) as Array<{ transaction_type: string; amount: number }>;

    const bookBalance = txRows.reduce((sum, row) => {
      if (row.transaction_type === 'DEPOSIT' || row.transaction_type === 'TRANSFER_IN') return sum + row.amount;
    ...
    const uncleared = db.prepare(`
      ...
      WHERE account_id = ?
        ...
        AND is_cleared = 0
    `).all(payload.account_id, payload.start_date, payload.end_date) as Array<{ id: string; amount: number }>;
  ...
  static markItemsCleared(reconciliationId: string, transactionIds: string[]) {
    const db = getDatabase();
    const tx = db.transaction((ids: string[]) => {
      for (const id of ids) {
        MoneyTransactionRepository.markCleared(id, true);
      }
  ...
  static getItems(reconciliationId: string) {
    const db = getDatabase();
    return db.prepare(`
