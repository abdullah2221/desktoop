...
class BranchRepository {
    ...
    static getAccessibleForUser(userId) {
        const db = (0, connection_1.getDatabase)();
    ...
    static userCanAccessBranch(userId, branchId) {
        const db = (0, connection_1.getDatabase)();
        const user = db.prepare('SELECT r.name as role_name FROM users u JOIN roles r ON r.id = u.role_id WHERE u.id=?').get(userId);
        if (user?.role_name === 'Owner/Admin')
            return true;
    ...
    `).run({
        ...
        if (payload.is_default)
            this.setDefault(id);
        if (info.changes > 0)
            AuditLogRepository_1.AuditLogRepository.write({ action: 'BRANCH_CREATE', user_id: actorId, details: `Branch ${id} created` });
    ...
    `).run({
        ...
        if (payload.is_default)
            this.setDefault(payload.id);
        if (info.changes > 0)
            AuditLogRepository_1.AuditLogRepository.write({ action: 'BRANCH_UPDATE', user_id: actorId, details: `Branch ${payload.id} updated` });
    ...
    static deactivate(id, actorId) {
        ...
        if (info.changes > 0)
            AuditLogRepository_1.AuditLogRepository.write({ action: 'BRANCH_DEACTIVATE', user_id: actorId, details: `Branch ${id} deactivated` });
        return info.changes > 0;
    }
    static setDefault(id) {
        const db = (0, connection_1.getDatabase)();
        const tx = db.transaction((branchId) => {
    ...
    static assignUserBranches(userId, branchIds, defaultBranchId) {
        const db = (0, connection_1.getDatabase)();
        const tx = db.transaction(() => {
            ...
            for (const branchId of branchIds)
                insert.run(userId, branchId, branchId === defaultBranchId ? 1 : 0);
            return true;
        });
        return tx();
    }
