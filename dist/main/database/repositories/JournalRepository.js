...
class JournalRepository {
    ...
    static getJournalById(id) {
        const db = (0, connection_1.getDatabase)();
        const journal = db.prepare('SELECT * FROM journal_entries WHERE id = ?').get(id);
        if (journal) {
    ...
    static createJournal(payload) {
      ...
      `);
            for (const line of data.lines) {
                ...
                const account = db.prepare('SELECT account_type FROM chart_of_accounts WHERE id = ?').get(line.account_id);
                if (account) {
                    let balanceChange = 0;
