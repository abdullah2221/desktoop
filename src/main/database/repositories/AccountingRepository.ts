...
export class AccountingRepository {
  ...
  static createAccount(payload: any) {
    const db = getDatabase();
  ...
  static updateAccount(payload: any) {
    const db = getDatabase();
    const stmt = db.prepare(`
    ...
    if (info.changes === 0) {
      ...
      const existing = db.prepare('SELECT is_system_account FROM chart_of_accounts WHERE id = ?').get(payload.id) as any;
  ...
  static deactivateAccount(id: string) {
    const db = getDatabase();
    // System accounts cannot be deleted/deactivated
    const existing = db.prepare('SELECT is_system_account FROM chart_of_accounts WHERE id = ?').get(id) as any;
    if (existing && existing.is_system_account) {
